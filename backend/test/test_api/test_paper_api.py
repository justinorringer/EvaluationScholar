...
def test_crud(client):
    ...
    #Check paper2
    resp = client.get(f'/api/papers/{p2_id}')
    assert resp.status_code == 200
    assert resp.json['name'] == 'name2'
    assert resp.json['year'] == 2002

    # Update paper1
    resp = client.put(f'/api/papers/{p1_id}', json={'name': 'name1_updated', 'year': 20011})
    assert resp.status_code == 200
    ...
    resp = client.put(f'/api/papers/{p2_id}', json={'name': 'name2_updated'})
    assert resp.status_code == 200
    ...
    # Delete paper1
    resp = client.delete(f'/api/papers/{p1_id}')
    assert resp.status_code == 200
...
def test_authors_paper_list(client):
    ...
    resp = client.get(f'/api/papers/{p1_id}/authors')
    assert resp.status_code == 200
    assert len(resp.json) == 0

    # Add author to paper1
    resp = client.put(f'/api/papers/{p1_id}/authors/{a1_id}')
    assert resp.status_code == 200

    # Check that author was added
    resp = client.get(f'/api/papers/{a1_id}/authors')
    assert resp.status_code == 200
    assert len(resp.json) == 1
    ...
    resp = client.put(f'/api/papers/{p1_id}/authors/{a2_id}')
    assert resp.status_code == 200
    ...
    # Delete author1
    resp = client.delete(f'/api/papers/{p1_id}/authors/{a1_id}')
    assert resp.status_code == 200
...
def test_edge_cases(client):
    ...
    resp = client.put('/api/papers/1', json={'name': 'name1_updated', 'year': 2001})
    assert resp.status_code == 404
    ...
    resp = client.delete(f'/api/papers/{p1_id}/authors/{a1_id - 1}')
    assert resp.status_code == 404

    # Add author to non-existing paper
    resp = client.put(f'/api/papers/{p1_id - 1}/authors/{a1_id}')
    assert resp.status_code == 404

    # Delete author from non-existing paper
    resp = client.delete(f'/api/papers/{p1_id - 1}/authors/{a1_id}')
    assert resp.status_code == 404

    # Add non-existing author to paper
    resp = client.put(f'/api/papers/{p1_id}/authors/{a1_id - 1}')
    assert resp.status_code == 404
